...
在 Claude Code 中，`hooks/setup.sh` 在会话启动时自动运行，并在二进制文件缺失时安装。初始克隆后无需手动步骤。
...
集成文件从二进制文件**同步**：缺失或过期的文件会被写入。`GEMINI.md` 和 `AGENTS.md` 仅在不存在时创建。
...
> **Web UI**：图可视化正在积极改进 — 聚类、邻居高亮和离线回退近期已上线。更多增强功能正在开发中。
