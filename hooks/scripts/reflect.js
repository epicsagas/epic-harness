...
import { runHook, hint, harnessExists, ensureDir, copyDirSync, rmDirSync, defaultMetrics, OBS_DIR, EVOLVED_DIR, EVOLVED_BACKUP_DIR, EVOLUTION_FILE, METRICS_FILE, GLOBAL_HARNESS_DIR, GLOBAL_PATTERNS_FILE, CROSS_PROJECT_OPT_IN_FILE, MAX_EVOLVED_SKILLS, STAGNATION_LIMIT, IMPROVEMENT_THRESHOLD, REPEATED_ERROR_MIN, FTB_LOOKAHEAD, FTB_MIN_CYCLES, DEBUG_LOOP_MIN, THRASH_MIN_EDITS, THRASH_MIN_ERRORS, WEAK_TOOL_RATE, WEAK_TOOL_MIN_OBS, WEAK_EXT_RATE, WEAK_EXT_MIN_OBS, HIGH_FREQ_ERROR_MIN, CWD, hashString, normalizeError, listDirs, listFiles, readJsonSafe, readJsonlSafe, appendJsonl, now, } from "./common.js";
import { existsSync as fsExistsSync } from "node:fs";
// ═══════════════════════════════════════════════════════
// Phase 1: Session Analysis (A-Evolve BatchAnalysis equivalent)
// ═══════════════════════════════════════════════════════
export function analyzeSession(observations) {
    ...
    for (const o of scored) {
        ...
        if (!toolMap[cat]) {
            toolMap[cat] = { tool_category: cat, total: 0, successes: 0, errors: 0, avg_score: 0, failure_categories: {} };
        }
    ...
    for (const o of scored) {
        ...
        if (!extMap[ext])
            extMap[ext] = { total: 0, errors: 0, success_rate: 0 };
        extMap[ext].total++;
        if (o.result === "error")
            extMap[ext].errors++;
    }
    ...
    const dimAvg = { tool_success: 0, output_quality: 0, execution_cost: 0 };
...
export function detectPatterns(observations) {
    ...
    // Same failure_category + same file + same error hash in consecutive observations (#4 dedup)
    {
        ...
        for (let i = 1; i < scored.length; i++) {
            ...
            const curr = scored[i];
            const currSnippet = curr.error_snippet ?? "";
            const prevSnippet = prev.error_snippet ?? "";
            ...
            if (curr.result === "error" && prev.result === "error" &&
            ...
            if ((o.tool_category === "edit" || o.tool_category === "write") && o.result === "success" && o.action) {
                ...
                for (let j = i + 1; j < Math.min(i + FTB_LOOKAHEAD + 1, scored.length); j++) {
                    ...
                    if (next.result === "error" && next.tool_category === "bash") {
                        const snippet = next.error_snippet ?? "";
    ...
    // Alternating edit-error on same file. O(n) single pass with per-file counters.
    {
        ...
        for (const o of scored) {
            ...
            if (!file)
                continue;
            if (!fileStats[file])
                fileStats[file] = { edits: 0, errors: 0 };
            if (o.tool_category === "edit" || o.tool_category === "write")
                fileStats[file].edits++;
            if (o.result === "error")
                fileStats[file].errors++;
        }
...
export function checkStagnation(metrics, currentScore) {
    const result = { shouldRollback: false, improved: false, rolledBackCount: 0 };
...
// ═══════════════════════════════════════════════════════
function seedSmartSkills(analysis, existingSkills) {
    let seeded = 0;
    if (existingSkills.length >= MAX_EVOLVED_SKILLS)
        return 0;
    ...
    for (const [_cat, stats] of Object.entries(analysis.per_tool_stats)) {
        ...
        if (successRate >= WEAK_TOOL_RATE || stats.total < WEAK_TOOL_MIN_OBS)
            continue;
...
function buildPatternSkill(pattern) {
    return [
        "---",
...
function buildExtSkill(ext, stats) {
...
function buildFailureCategorySkill(category, count) {
    const remediations = {
...
function buildSummary(analysis) {
    ...
    if (topErrors.length > 0)
        parts.push(`errors=[${topErrors.join(",")}]`);
...
// ═══════════════════════════════════════════════════════
function updateSkillAttribution(metrics, analysis, evolvedSkills) {
    if (!metrics.skill_attribution)
        metrics.skill_attribution = {};
    for (const skill of evolvedSkills) {
        ...
        attr.avg_score_with = Math.round(((attr.avg_score_with * (attr.sessions_active - 1)) + analysis.avg_score) / attr.sessions_active * 1000) / 1000;
    }
    ...
    for (const [name, attr] of Object.entries(metrics.skill_attribution)) {
        ...
        if (sessionsWithout > 0 && metrics.score_history.length > 0) {
            ...
            attr.avg_score_without = Math.round(((totalAvg * totalSessions) - (attr.avg_score_with * attr.sessions_active)) / sessionsWithout * 1000) / 1000;
        }
    }
...
// ═══════════════════════════════════════════════════════
function exportToGlobalPatterns(analysis, patterns) {
    // Only export if opt-in
    if (!fsExistsSync(CROSS_PROJECT_OPT_IN_FILE))
        return;
...
if (isMain)
    runHook(() => {
        ...
        for (const f of obsFiles) {
            const records = readJsonlSafe(join(OBS_DIR, f));
        ...
        const lastErrors = observations
            ...
            .slice(-3)
            .map(o => `${o.failure_category}: ${o.error_snippet?.slice(0, 100) ?? o.action}`)
            .join(" | ");
        if (lastErrors)
            metrics.last_error_context = lastErrors;
        // 10. Update metrics
        const scoreEntry = {
            timestamp: now(),
            success_rate: analysis.success_rate,
        ...
        metrics.avg_success_rate = Math.round(((metrics.avg_success_rate * (metrics.total_sessions - 1)) + analysis.success_rate) / metrics.total_sessions * 1000) / 1000;
        ...
        if (weakTools.length > 0)
            hint("reflect", `Weak tools: ${weakTools.join(", ")}`);
        ...
        if (weakExts.length > 0)
            hint("reflect", `Weak file types: ${weakExts.join(", ")}`);
        if (analysis.failure_patterns.length > 0) {
        ...
        if (seeded > 0)
            hint("reflect", `Evolved ${seeded} new skill(s)`);
        if (stagnation.shouldRollback)
            hint("reflect", `Rolled back ${stagnation.rolledBackCount} stagnant skills`);
