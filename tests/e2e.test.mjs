...
function setupHarness() {
  ...
  mkdirSync(join(dir, ".harness/obs"), { recursive: true });
  mkdirSync(join(dir, ".harness/sessions"), { recursive: true });
...
test("observe: appends scored record to session JSONL", () => {
  ...
  try {
    ...
    const obsDir = join(dir, ".harness/obs");
...
test("observe: classifies type_error from stderr", () => {
  ...
  try {
    ...
    const obsDir2 = join(dir, ".harness/obs");
...
test("snapshot: writes snapshot file with obs summary", () => {
  ...
  try {
    ...
    const sessionsDir = join(dir, ".harness/sessions");
