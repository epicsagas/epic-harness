...
fn scenario_seesaw_catches_regression() {
    ...
    let regressed_score = evolve::seesaw::scores_from_digests(&regression_digests)
        .get("PIPE-1")
        .copied();
    assert!(
    ...
    reg.update(&evolve::seesaw::scores_from_digests(&seed_digests));
...
fn scenario_outcome_score_bounds() {
    ...
    let s = evolve::seesaw::scores_from_digests(&[TaskDigest {
        task_id: "t".into(),
    ...
    assert_eq!(
        evolve::seesaw::scores_from_digests(&[TaskDigest {
            task_id: "t".into(),
    ...
    let partial = evolve::seesaw::scores_from_digests(&[TaskDigest {
        task_id: "t".into(),
    ...
    let zero_total = evolve::seesaw::scores_from_digests(&[TaskDigest {
        task_id: "t".into(),
        outcome: TaskOutcome::PartialFailure {
            failed_steps: 0,
            total_steps: 0,
