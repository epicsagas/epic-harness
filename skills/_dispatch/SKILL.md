...
1. Create `$(epic-harness path)/dispatch/dispatch_YYYYMMDD.jsonl` if it doesn't exist
...
Check `$(epic-harness path)/evolved/` for project-specific auto-evolved skills. These are generated by the Ring 3 evolution loop based on actual failure patterns.

### How to scan evolved skills:

1. List directories in `$(epic-harness path)/evolved/` — each is a skill
