...
/// null bytes stripped, C0 chars (U+0001–U+001F) → `\\xHH` (YAML 1.2 §6.8),
/// C1 controls (U+0080–U+009F) stripped, Plane-14 U+E0000–U+E01EF stripped.
...
fn yaml_quote(s: &str) -> String {
    ...
    for c in s.chars() {
        match c {
            ...
            c if (c as u32) >= 0x80 && (c as u32) <= 0x9F => {} // strip C1 controls
            c if ('\u{E0000}'..='\u{E01EF}').contains(&c) => {} // strip Plane-14 injection chars
            c => out.push(c),
        }
    }
    out.push('"');
...
/// content into it.  Strips: null bytes, lines beginning with `---`, and the
/// Plane-14 range U+E0000–U+E01EF (Tags block + adjacent Reserved region +
/// Variation Selectors Supplement) used as LLM prompt-injection vectors.
pub(crate) fn sanitize_mission(mission: &str) -> String {
    mission
        ...
        .map(|l| {
            l.chars()
                .filter(|&c| !('\u{E0000}'..='\u{E01EF}').contains(&c))
                .collect::<String>()
        })
        .collect::<Vec<_>>()
        .join("\n")
...
/// Strip line-break characters, null bytes, and Plane-14 invisible Unicode
/// (U+E0000–U+E01EF) from a value destined for Markdown body text.
...
/// Handles the sequences that `yaml_quote` produces for printable input:
/// `\\` → `\`, `\"` → `"`, `\t` → tab.  `\xHH` sequences are passed through as
/// literal text (safe for display; real C0 chars are stripped by the output filter).
/// Intended for display-only paths (e.g. `epic team show`).
pub(crate) fn yaml_unescape_display(s: &str) -> String {
    ...
    // Strip C0 controls (except tab), DEL (U+007F), 8-bit C1 controls
    // (U+0080–U+009F, incl. U+009B CSI), and the Plane-14 range U+E0000–U+E01EF —
    // a single RangeInclusive that covers the Tags block (E0000–E007F), the adjacent
...
mod tests {
    ...
    fn test_yaml_quote_escapes() {
        ...
        // C1 controls (U+0080–U+009F) stripped on write path
        assert_eq!(yaml_quote("a\u{0080}b"), "\"ab\"");
        assert_eq!(yaml_quote("a\u{009B}b"), "\"ab\""); // CSI
        assert_eq!(yaml_quote("a\u{009F}b"), "\"ab\""); // last C1
        // Plane-14 Tags/VarSel (U+E0000–U+E01EF) stripped on write path
        assert_eq!(yaml_quote("a\u{E0001}b"), "\"ab\"");
        assert_eq!(yaml_quote("a\u{E0080}b"), "\"ab\""); // Reserved region
        assert_eq!(yaml_quote("a\u{E0100}b"), "\"ab\""); // VarSel Supplement floor
    }
    ...
    fn test_yaml_unescape_display() {
        ...
        // \t is decoded to a real tab (yaml_quote emits \t for tab chars in description values)
        assert_eq!(yaml_unescape_display("\"a\\tb\""), "a\tb");
        // unknown escape sequences are preserved with the backslash (e.g. \x is not decoded)
        assert_eq!(yaml_unescape_display("\"a\\xb\""), "a\\xb");
        // genuine \xHH two-hex-digit form (as yaml_quote emits for C0 chars) — passed through as literal text
        assert_eq!(yaml_unescape_display("\"a\\x01b\""), "a\\x01b");
        assert_eq!(yaml_unescape_display("\"a\\x1Fb\""), "a\\x1Fb");
        // DEL (U+007F) stripped
        assert_eq!(yaml_unescape_display("\"\x7F\""), "");
        // C1 controls (U+0080–U+009F) including CSI (U+009B) stripped
        assert_eq!(yaml_unescape_display("\"\u{009B}31m\""), "31m");
        assert_eq!(yaml_unescape_display("\"\u{0080}\""), "");
        // Plane-14 range U+E0000–U+E01EF stripped (Tags + Reserved + VarSel Supplement)
        assert_eq!(yaml_unescape_display("\"\u{E0000}\""), ""); // Tags floor
        assert_eq!(yaml_unescape_display("\"\u{E0001}hello\""), "hello");
        assert_eq!(yaml_unescape_display("\"\u{E007F}\""), ""); // Tags ceiling
        assert_eq!(yaml_unescape_display("\"\u{E0080}\""), ""); // Reserved region (E0080–E00FF) stripped
        assert_eq!(yaml_unescape_display("\"\u{E00FF}\""), ""); // Reserved ceiling
        // Variation Selectors Supplement (U+E0100–U+E01EF) stripped
        assert_eq!(yaml_unescape_display("\"\u{E0100}\""), ""); // VarSel floor
        assert_eq!(yaml_unescape_display("\"\u{E01EF}\""), ""); // VarSel ceiling
        assert_eq!(yaml_unescape_display("\"\u{E0100}hi\""), "hi");
        // BMP Private Use Area (U+E000, 4-digit hex ≠ Tags block) is allowed
        assert_eq!(yaml_unescape_display("\"\u{E001}\""), "\u{E001}");
        // Normal Unicode above U+00A0 (accented, CJK) is allowed
