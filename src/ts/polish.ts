...
import {
  ...
  type HookInput, type ObsRecord, type ScoreDimensions,
} from "./common.js";

function tryExec(cmd: string, opts?: { silent?: boolean }): string | null {
  try {
...
function feedbackToObserve(filePath: string, formatter: string, success: boolean, errorSnippet?: string): void {
  ...
  const score = Math.round(
    (SCORE_WEIGHTS.success * dims.tool_success +
     SCORE_WEIGHTS.quality * dims.output_quality +
     SCORE_WEIGHTS.cost * dims.execution_cost) * 1000
  ) / 1000;
...
function checkTS(filePath: string): void {
...
runHook((input: HookInput) => {
  ...
  if ([".js", ".jsx", ".ts", ".tsx"].includes(ext)) {
    formatJS(filePath);
    if ([".ts", ".tsx"].includes(ext)) checkTS(filePath);
  } else if (ext === ".py") {
