...
import {
  runHook, hint, raw, harnessExists, ensureDir,
  CWD, HARNESS_DIR, SESSIONS_DIR, MEMORY_DIR, EVOLVED_DIR, TEAM_DIR,
  OBS_DIR, METRICS_FILE, PRESETS_DIR, GLOBAL_HARNESS_DIR, GLOBAL_PATTERNS_FILE,
  CROSS_PROJECT_OPT_IN_FILE,
  readJsonSafe, readJsonlSafe, listDirs, listFiles, defaultMetrics,
  type SessionSnapshot, type Metrics,
} from "./common.js";
...
const PRESETS: Record<string, { skills: Record<string, string> }> = {
  "Node.js": {
    skills: {
      "evo-ts-care": [
        "---",
...
function applyColdStartPresets(stacks: string[]): number {
  let applied = 0;
  for (const stack of stacks) {
    ...
    for (const [skillName, content] of Object.entries(preset.skills)) {
      ...
      if (existsSync(skillDir)) continue;  // don't overwrite existing
      ensureDir(skillDir);
...
function getCrossProjectHints(): string[] {
  if (!existsSync(CROSS_PROJECT_OPT_IN_FILE)) return [];
  if (!existsSync(GLOBAL_PATTERNS_FILE)) return [];
  ...
  const weakToolCounts: Record<string, number> = {};
  for (const r of otherPatterns.slice(-20)) {
    const tools = (r.weak_tools as string[]) ?? [];
...
runHook(() => {
  ...
  if (snapshots.length > 0) {
    const latest = readJsonSafe<SessionSnapshot>(
      join(SESSIONS_DIR, snapshots[snapshots.length - 1]),
      { timestamp: "", type: "", summary: "", pending_tasks: [], context_usage: null },
    );
    if (latest.summary) hint("resume", `Previous: ${latest.summary}`);
  ...
  if (metrics.total_sessions > 0) {
    ...
    if (lastEntry?.dimension_averages) {
      ...
      const weak: string[] = [];
      if (dims.tool_success < 0.7) weak.push(`tool_success=${dims.tool_success}`);
      if (dims.output_quality < 0.7) weak.push(`output_quality=${dims.output_quality}`);
      if (weak.length > 0) {
  ...
  if (evolved.length > 0) hint("resume", `Evolved skills: ${evolved.join(", ")}`);
  ...
  if (memFiles.length > 0) hint("resume", `Memory: ${memFiles.length} file(s)`);

  // 6. Stack detection
  const stacks = Object.entries(STACK_FILES)
    .filter(([f]) => existsSync(join(CWD, f)))
    .map(([, s]) => s);
  if (stacks.length > 0) hint("resume", `Stack: ${stacks.join(", ")}`);
