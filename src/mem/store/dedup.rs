...
pub fn write_node_dedup(node: &Node, window_hours: u64) -> io::Result<(String, bool)> {
    ...
    runtime::block_on(write_node_dedup_async(&pool, node, window_hours))
}

/// Async core — shared by sync wrapper and pool variant.
pub(crate) async fn write_node_dedup_async(
    pool: &sqlx::AnyPool,
    node: &Node,
    window_hours: u64,
...
/// Async write-with-dedup using pool.
pub async fn write_node_dedup_pool(
    pool: &sqlx::AnyPool,
    node: &Node,
    window_hours: u64,
) -> io::Result<(String, bool)> {
    write_node_dedup_async(pool, node, window_hours).await
}
