...
<aside class="flex flex-col h-full w-64 border-r border-outline-variant bg-surface-container-lowest shrink-0 overflow-y-auto">
    ...
    <div class="mt-6 space-y-1">
      {#each navItems as item}
        <button
          ...
          class="w-full flex items-center gap-3 p-3 rounded-lg transition-colors text-body-md
            {isActive(item.path)
  ...
  <div class="mt-auto p-4 space-y-1 border-t border-outline-variant">
    <button class="w-full flex items-center gap-3 p-3 rounded-lg text-on-surface-variant hover:bg-surface-variant/50 transition-colors">
